...
type WealthPrimitive = string | number | boolean | null;
export type WealthValue =
  | WealthPrimitive
  | WealthValue[]
  | { [key: string]: WealthValue | undefined };
...
type WealthResponse = {
  ...
  verdict?: string | null;
  summary?: WealthValue;
  analysis?: WealthValue;
  timing?: WealthValue;
  metadata?: WealthValue;
  raw_data?: WealthValue;
};

type SectionKey = 'summary' | 'analysis' | 'timing' | 'metadata' | 'raw_data';
...
function isEmptyValue(value: WealthValue | undefined | null): boolean {
  if (value === null || value === undefined) return true;
  if (typeof value === 'string') return value.trim().length === 0;
  if (Array.isArray(value)) return value.every((item) => isEmptyValue(item as WealthValue));
  if (typeof value === 'object')
    return Object.values(value).every((entry) => isEmptyValue(entry as WealthValue));
  return false;
}
...
function renderValue(value: WealthValue, t: any, level = 0): JSX.Element {
  if (value === null || value === undefined) {
    return <p className="text-sm italic text-slate-400">{t('wealth.failed')}</p>;
  }
      ...
      <ul className="mt-2 list-disc space-y-3 pl-5 text-slate-100">
        {value.map((item, index) => (
          <li key={`list-${level}-${index}`} className="space-y-2">
            {renderValue(item as WealthValue, t, level + 1)}
          </li>
        ))}
      </ul>
    );
  ...
  if (typeof value === 'object') {
    const entries = Object.entries(value).filter(
      ([, entryValue]) => !isEmptyValue(entryValue as WealthValue),
    );
    if (!entries.length) {
      return <p className="text-sm italic text-slate-400">{t('wealth.failed')}</p>;
    }
        ...
        {entries.map(([key, entryValue]) => (
          ...
          >
            ...
            <dd className="mt-2 text-slate-100">
              {renderValue(entryValue as WealthValue, t, level + 1)}
            </dd>
          </div>
        ))}
      </dl>
...
function formatTimestamp(value?: string | null): string | null {
  ...
  try {
    ...
    console.warn('[Wealth] Failed to format timestamp', err);
    return value;
  }
}
...
export default function WealthPage() {
  ...
  const [wealthData, setWealthData] = useState<WealthResponse | null>(null);
  const [loadingData, setLoadingData] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const profileId = profile?.id ? Number(profile.id) : null;
  const aiSummary = useAiSummary('wealth', profileId);

  const SECTION_CONFIG: SectionConfig[] = useMemo(() => [
    {
      key: 'summary',
      title: t('wealth.summaryTitle'),
      blurb: t('wealth.summaryBlurb'),
    },
    {
      key: 'analysis',
      title: t('wealth.analysisTitle'),
      blurb: t('wealth.analysisBlurb'),
    },
    {
      key: 'timing',
      title: t('wealth.timingTitle'),
      blurb: t('wealth.timingBlurb'),
    },
    {
      key: 'metadata',
      title: t('wealth.metadataTitle'),
      blurb: t('wealth.metadataBlurb'),
    },
    {
      key: 'raw_data',
      title: t('wealth.rawDataTitle'),
      blurb: t('wealth.rawDataBlurb'),
    },
  ], [t]);

  const fetchWealth = useCallback(
    async (forceRefresh: boolean = false) => {
      ...
      try {
        ...
        const url = `${API_BASE}/api/fortune/wealth?${params.toString()}`;
        ...
        if (response.status === 404) {
          setWealthData(null);
          setError('Wealth intelligence is not available yet.');
          return;
        }
        if (!response.ok) {
          setWealthData(null);
          setError('Failed to load wealth intelligence.');
          return;
        }
        const payload = (await response.json()) as WealthResponse;
        setWealthData(payload);
      } catch (err) {
        console.error('[Wealth] fetch failed', err);
        setWealthData(null);
        setError('Failed to load wealth intelligence.');
      } finally {
  ...
  useEffect(() => {
    ...
    if (!profileId) return;
    fetchWealth();
  }, [profileId, profileLoading, loadingAuth, fetchWealth]);

  const sections = useMemo(
    () =>
      SECTION_CONFIG.map((section) => ({
        ...section,
        value: (wealthData?.[section.key] ?? null) as WealthValue | null,
      })),
    [wealthData, SECTION_CONFIG],
  );

  const updatedAt = formatTimestamp(wealthData?.updated_at);
  const wealthIndex = wealthData?.wealth_index;
  const verdict = wealthData?.verdict;
      ...
      <div className="mx-auto flex min-h-screen max-w-screen-xl flex-col gap-8 px-4 py-10 text-white sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl border border-neon-cyan/50 bg-gradient-to-br from-neo-dark via-[#0b1b36] to-neo-dark p-8 shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col items-center gap-6 text-center">
            <div className="relative h-48 w-48 overflow-hidden rounded-full border border-neon-cyan/60 shadow-[0_0_25px_rgba(0,255,255,0.35)]">
              <Image src="/logo/logo.png" alt="Wealth mandala" fill className="object-cover" priority />
            </div>
            <div className="space-y-3">
              ...
              <p className="mx-auto max-w-2xl text-base text-slate-200">
                Trace how MitraVeda decodes your prosperity path. Explore the yogas, timing windows, and
                karmic levers that shape sustainable abundance.
              </p>
            </div>
            ...
            <div className="mt-2 flex flex-col items-center gap-3 text-sm text-slate-300 sm:flex-row">
              ...
              {typeof wealthIndex === 'number' && (
                <span className="rounded-full border border-neon-cyan/30 bg-neo-dark/60 px-4 py-2 text-slate-200">
                  Wealth Index {wealthIndex.toFixed(1)}
                </span>
              )}
              {verdict && (
                <span className="rounded-full border border-neon-cyan/30 bg-neo-dark/60 px-4 py-2 text-slate-200">
        ...
        {loadingData && (
          <div className="rounded-3xl border border-neon-cyan/30 bg-neo-dark/70 p-6 text-neon-cyan">
            Synchronising MitraVeda wealth layers...
          </div>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 space-y-4 text-base">
                    {hasContent ? (
                      renderValue((section.value ?? null) as WealthValue, t)
                    ) : (
                      <p className="text-sm italic text-slate-400">{t('wealth.failed')}</p>
                    )}
                  </div>
                </section>
              );
        ...
        {!error && !wealthData && !loadingData && (
          <div className="rounded-3xl border border-neon-cyan/40 bg-neo-dark/70 p-6 text-slate-200">
            Wealth intelligence is being prepared. Please revisit shortly.
          </div>
        )}
      </div>
    </div>
